...
const PokeClient = ({ hover, poke, favorite, favHandler, getFavs }: any) => {
  return (
    <div
      className={
        hover
          ? "p-2 absolute cursor-pointer transition ease-in delay-100 duration-300 opacity-0 hover:opacity-100 top-0 left-0 w-full h-full text-white bg-neutral bg-opacity-70"
          : "hidden"
      }>
      <section className='flex justify-between items-top'>
        ...
        <button
          className={`w-10 h-10 bg-transparent btn btn-ghost p-[4px] ${
            favorite ? "fill-primary" : "fill-current"
          }`}
          onClick={favHandler}>
          <svg
